...
public class ParserAstTests {
    ...
    @Test
    public void operatorPrecedence2() throws Exception {
        String input = "class Main { public static void main(String[] args) { boolean x; x = 5 + 2 * 3 < 10 - 9 / 3; }}";
        MJProgram ast = parse(input);
        String printed = AstPrinter.print(ast);
        Assert.assertThat(printed, CoreMatchers.containsString("x = ((5 + (2 * 3)) < (10 - (9 / 3)))"));
    }

    @Test
    public void operatorPrecedence3() throws Exception {
        String input = "class Main { public static void main(String[] args) { boolean x; x = !4 < 3; }}";
        MJProgram ast = parse(input);
        String printed = AstPrinter.print(ast);
        Assert.assertThat(printed, CoreMatchers.containsString("x = ((! 4) < 3)"));
    }
        ...
        ast.accept(new MJElement.DefaultVisitor() {
            ...
            public void visit(MJVarDecl v) {
                if (v.getName().equals("ar")) {
                    Assert.assertThat("Type of ar must be an array type", v.getType(), CoreMatchers.instanceOf(MJTypeArray.class));
                }
            }

            @Override
            public void visit(MJNewArray newArray) {
                Assert.assertThat("Basetype of new-array expression must be an array type", newArray.getBaseType(), CoreMatchers.instanceOf(MJTypeArray.class));
            }
        });
    ...
    @Test
    public void testArrayAccessParens() throws Exception {
        String input = "class Main{public static void main(String[] a){ int[] ar; (ar)[1] = 5; }}";
        MJProgram ast = parse(input);
        String printed = AstPrinter.print(ast);
        Assert.assertThat(printed, CoreMatchers.containsString("ar[1] = 5"));
    }
    ...
    @Test(expected = frontend.SyntaxError.class)
    public void methodCallAdditionalCommaFront() throws Exception {
        String input = "class Main{public static void main(String[] a){ a.m(,x, y); }}";
        MJProgram ast = parse(input);
    }

    @Test(expected = frontend.SyntaxError.class)
    public void parameterAdditionalCommaFront() throws Exception {
        String input = "class Main{public static void main(String[] a){}}  class C { int m(,int x) { return 5; } }";
        MJProgram ast = parse(input);
    }
