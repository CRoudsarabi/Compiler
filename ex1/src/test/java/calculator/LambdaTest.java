...
/**
 * This is a test for the optional exercise.
 * It tests lambda expressions and let expressions.
 * <p>
 * This test has the same prerequisites as the InterpreterTest
 */
public class LambdaTest {

    /*** The y-combinator is a fixpoint operator.
     * It satisfies the equation Y f = f (Y f).
     */
    private static final String yCombinator = "let Y = fun f -> (fun  x -> f (x x)) (fun x -> f (x x)) in \n";
    /*** The z-combinator is a fixpoint operator like the y-combinator
     *  but it also works with strict evaluation.
     */
    ...
    public void yCombinator() {
        String expr = yCombinator +
                "let facEq = fun f -> fun n -> " +
                "  if n==0 then 1 " +
                "  else n * f (n-1) in " +
                "let fac = Z facEq in " +
                "fac(5)";
